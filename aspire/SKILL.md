---
name: aspire
description: "Use when working with an Aspire distributed application: operate an AppHost or resources through the Aspire CLI; start, stop, restart, or wait for resources; inspect app state, logs, traces, docs, or health; add integrations; manage secrets/config; publish/deploy or run pipeline steps; initialize an existing app; find frontend URLs for Playwright; expose custom dashboard/resource commands; or understand Aspire AppHost APIs in C#. Use even if the user says AppHost, resources, dashboard, bootstrap, Playwright URL, or local distributed app workflow without naming Aspire. Do not use for non-Aspire .NET apps, container-only repos without an AppHost, or ordinary build/test tasks."
---
...
First check if the aspire command is installed by running `aspire start` is not found then refer to [references/install.md](references/install.md)

Resources are typically defined in an AppHost such as, `AppHost.cs` or `AppHost/AppHost.csproj (Program.cs)`.
...
- Understanding unfamiliar Aspire AppHost APIs before editing C# AppHosts
...
- For installation of the aspire command, the aspire cli, see [references/install.md](references/install.md).
